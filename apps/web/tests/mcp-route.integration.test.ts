...
describe.skipIf(!probe.reachable)('/mcp with a real agent token', () => {
  ...
  it('initialises and lists the seventeen tools for a valid token', async () => {
    process.env.MCP_HTTP_ENABLED = 'true';
    const { POST } = await loadRoute();
    ...
    expect(payload.result.tools).toHaveLength(17);
    ...
    expect(payload.result.tools.map((tool) => tool.name)).toContain('wiki.get_rules');
    expect(payload.result.tools.map((tool) => tool.name)).toContain('wiki.list_skills');
    expect(payload.result.tools.map((tool) => tool.name)).toContain('wiki.get_skill');
  });
