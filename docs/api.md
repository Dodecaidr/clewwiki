...
| `GET /api/v1/spaces/{key}/export` | session or token | `pages:read` | The whole space as a ZIP of Markdown files mirroring the tree. Takes `format=md`. `files=latest` adds the latest version of every file beside its page (`KEY/path.files/name`) and `KEY/_files.json` listing what is in the archive and what was left out, streamed; the archive stops short of 4 GiB. |
| `POST /api/v1/spaces/{key}/imports` | admin or editor session | — | Start an import. JSON for Confluence (`deployment` — `cloud` (default) or `datacenter` — `base_url`, `space_key`, `email`, `api_token`; the last two are used once and never stored, are required on Cloud, and on a Data Center may be a personal access token alone, a username and password, or both empty for a space that is readable without signing in); `multipart/form-data` with `source` and `file` for a Notion ZIP, a Markdown ZIP or a PDF. Answers `201` with the import in `needs_review`; nothing is written to pages. `stats.images` and `stats.images_skipped` count the images — of a Notion or Markdown archive, or downloaded from Confluence — that will and will not be carried across; `stats.files` and `stats.file_versions` count the other Confluence attachments staged as files, which are downloaded into the file store while the import is read. `413` when the declared `Content-Length` is over 200 MB and `411` when it is absent; `400 validation` for a Confluence address that is not an `https` host this instance may reach — public always, a private network only for a host name the operator listed in `IMPORT_CONFLUENCE_PRIVATE_HOSTS`. Refused to agent tokens. |
...
| `POST /api/v1/imports/{id}/apply` | admin or editor session | — | Create the pages. Answers with what landed and what was skipped, with the reason — a taken path or somebody else's claim. A created page says how many `images` and `files` came with it, and `images_failed` or `files_failed` when something staged could not be stored after all. |
...
| *(roles)* | session | — | A signed-in person's role stands in for a scope list. An **editor** or **administrator** session has every scope. A **viewer** session has `identity:read` and `pages:read`: every endpoint in this table that needs `pages:write` or `pages:delete` answers a viewer `403 forbidden`, exactly as it answers a read-only token `403 insufficient_scope`. `POST /api/v1/inbox/read` and the watch endpoints need only `pages:read`, so a viewer keeps their own read mark and chooses what to watch. |
| `GET /api/v1/inbox` | session or token | `pages:read` | The caller's inbox: what others said or decided, since the caller last marked it read, about things the caller had a hand in — messages and comments that mention them by name (`kind: "mention"`), messages in discussions they opened or spoke in, those discussions being resolved, replies in their comment threads, new comments on a page as they left it, reviews of their changes, new versions of files on pages and in spaces they watch (`kind: "file.version"`, with `file`: `file_id`, `name`, `version`), and changes to pages they watch (`kind: "page.updated"`, one item per page, with `changes`: `count`, `version`). `unread=true` for the unread only, `limit` up to 100. Computed when asked, under the caller's current visibility; nothing is stored per event. Goes back 30 days. |
| *(mentions)* | — | — | `POST` on a space's discussions, on a discussion's messages and on a page's or a thread's comments read the text for `@name` and `@[Name With Spaces]`, resolve each against the workspace's members and live tokens, and answer with `mentioned: [{ type, label }]` on the message or comment they created — who was reached. An unmatched name is not an error. |
| `POST /api/v1/inbox/read` | session or token | `pages:read` | Move the caller's read mark to `up_to` (ISO 8601, default now). Forward only, never past now. It writes the caller's own bookmark and nothing else, which is why it needs no write scope. |
...
| `GET /api/v1/pages/{id}/files` | session or token | `pages:read` | The files attached to a page, by name, each with its latest version (`latest`: `version`, `bytes`, `sha256`, `content_type`, `note`, `created_by`) and `url`, its permanent address. `uploads_enabled` is false when `FILES_DRIVER=off`; files attached before stay listed. |
| `PUT /api/v1/pages/{id}/files/{name}` | session or token | `pages:write` | Upload the request body as the file `name`: a new file, or the next version of the file the page already has by that name, in any case. Any type; `Content-Length` required. `?note=` says what changed. `201` with the file and the new `version`; `200` when these bytes already are the latest version, so a retry adds nothing. `413` past `FILES_MAX_UPLOAD_MB`, `411` without a size, `400 validation` for a name with a slash, control or text-direction characters, an empty body, or one that ended short of its declared size; `409 conflict` when the workspace's store is full; `403` when files are off. A session uploads with `PUT` from this origin only. Takes no claim. |
| `GET /api/v1/pages/{id}/files/{name}` | session or token | `pages:read` | Download the file: its latest version, or `?version=N`. The address to put in a page or a release note — it does not change while the file exists. Always an attachment, with `nosniff`, a sandboxing `Content-Security-Policy` and `Cross-Origin-Resource-Policy: same-origin`; `ETag` is the `sha256`, a single `Range` is honoured (`206`, `416`). `X-File-Version` names the version served. |
| `GET /api/v1/files/{fileId}` | session or token | `pages:read` | A file with every version, newest first, without the bytes. `404` for a file of a deleted page. |
| `GET /api/v1/files/{fileId}/content` | session or token | `pages:read` | The bytes of one version, served as above: the latest, or `?version=N`. |
| `POST /api/v1/files/{fileId}/restore` | session or token | `pages:write` | Go back to `version` (optional `note`) by adding it as a new version with `restored_from`: nothing anybody downloaded changes. `201`, or `200` when that content already is the latest. |
| `DELETE /api/v1/files/{fileId}` | session or token | `pages:write` + `pages:delete` | Remove a file and all its versions for good. The bytes go with the next hourly sweep unless another file holds the same content. |
| `GET /api/v1/watches` | session or token | `pages:read` | What the caller watches: `kind` (`page` or `space`), `page_id`, `space`, `title`. |
| `POST /api/v1/watches` | session or token | `pages:read` | Watch `{"page_id"}` or `{"space": "KEY"}`: `201`, or `200` if already watching. What somebody else does afterwards appears in the inbox — a page's changes as `page.updated` and new file versions as `file.version`; a space reports file versions only. A viewer may watch. |
| `DELETE /api/v1/watches` | session or token | `pages:read` | Stop watching the same body. |
