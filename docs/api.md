...
| `POST /api/v1/imports/{id}/apply` | admin or editor session | — | Create the pages. Answers with what landed and what was skipped, with the reason — a taken path or somebody else's claim. A created page says how many `images` came with it, and `images_failed` when a staged image could not be stored after all. |
